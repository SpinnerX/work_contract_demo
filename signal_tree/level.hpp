...
#include <signal_tree/node.hpp>

namespace work_contracts{
    template<size_t N0, size_t N1> requires((std::popcount(N0) == 1) and (std::popcount(N1) == 1))
    struct level_traits{
    ...
    public:
        ...
        std::pair<signal_index, bool> select(bias_flags) noexcept requires(root_level_traits<T>);
    ...
    protected:
        ...
        template<template<uint64_t, uint64_t> class>
        std::pair<signal_index, bool> select(bias_flags, node_index) noexcept;
...
//=============================================================================
template<work_contracts::level_traits_concept T>
inline bool work_contracts::level<T>::empty() const noexcept requires (root_level_traits<T>){
    return nodes_[0].empty();
}

template<work_contracts::level_traits_concept T>
inline std::pair<bool, bool> work_contracts::level<T>::set(signal_index signalIndex) noexcept {
...
template<work_contracts::level_traits_concept T>
template<template<uint64_t, uint64_t> class select_function>
inline auto work_contracts::level<T>::select(bias_flags biasFlags) noexcept -> std::pair<signal_index, bool> requires(root_level_traits<T>){
...
template<work_contracts::level_traits_concept T>
template<template<uint64_t, uint64_t> class select_function>
inline auto work_contracts::level<T>::select(bias_flags biasFlags, node_index nodeIndex) noexcept -> std::pair<signal_index, bool> {
